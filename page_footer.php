    ...
    <script>
    // --- LOGICA DI ANIMAZIONE DELLA NAVBAR MOBILE ---
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function positionIndicator(element) {
            if (!element || !navIndicator || !mobileNav) return;
            const itemRect = element.getBoundingClientRect();
            const navRect = mobileNav.getBoundingClientRect();
            // Calcola il centro dell'icona e posiziona l'indicatore lì
            const offset = itemRect.left - navRect.left + (itemRect.width - navIndicator.offsetWidth) / 2;
            navIndicator.style.transform = `translateX(${offset}px)`;
        }

        const activeItem = document.querySelector('.nav-item.active');
        if (activeItem) {
            // Posiziona l'indicatore all'avvio senza animazione
            navIndicator.style.transition = 'none';
            positionIndicator(activeItem);
            // Ripristina l'animazione dopo un breve ritardo
            setTimeout(() => {
                navIndicator.style.transition = 'transform 0.4s cubic-bezier(0.68, -0.55, 0.27, 1.55)';
            }, 50);
        }

        // Riposiziona l'indicatore al resize della finestra per mantenere l'allineamento
