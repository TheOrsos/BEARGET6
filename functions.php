...
function find_user_by_friend_code($conn, $friend_code) {
    $sql = "SELECT id, username, subscription_status FROM users WHERE friend_code = ? LIMIT 1";
    $stmt = $conn->prepare($sql);
...
function share_note_with_user($conn, $note_id, $sharer_user_id, $friend_code, $permission) {
    ...
    if (check_block_status($conn, $sharer_user_id, $friend_id)) {
        return ['success' => false, 'message' => 'Azione non permessa. Uno degli utenti ha bloccato laltro.'];
    }
    // --- FINE CONTROLLO ---
...
function create_notification($conn, $user_id, $type, $message, $related_id) {
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            error_log("Fallimento esecuzione in create_notification: " . $stmt->error);
            $stmt->close();
            return false;
        }
    } else {
        error_log("Fallimento preparazione in create_notification: " . $conn->error);
        return false;
    }
}
...
function get_all_notifications($conn, $user_id) {
    $sql = "SELECT n.*, lr.status as loan_status
            FROM notifications n
            LEFT JOIN loan_requests lr ON n.type = 'loan_request' AND n.related_id = lr.id
            WHERE n.user_id = ? 
...
function get_expenses_by_category($conn, $user_id, $filters = []) {
    ...
    // --- NUOVA LOGICA FILTRO ETICHETTE ---
    if (!empty($filters['tag_id'])) {
        if ($filters['tag_id'] === '_any_') {
            $sql .= " AND EXISTS (SELECT 1 FROM transaction_tags tt WHERE tt.transaction_id = t.id)";
        } elseif ($filters['tag_id'] === '_none_') {
...
function get_income_expense_trend($conn, $user_id, $filters = []) {
    ...
    foreach ($period as $dt) {
        ...
        $sql = "SELECT t.type, SUM(t.amount) as total FROM transactions t WHERE t.user_id = ? AND t.transaction_date BETWEEN ? AND ?";
...
function get_net_worth_trend($conn, $user_id, $filters = []) {
    ...
    $tag_id_filter = $filters['tag_id'] ?? null;

    // --- LOGICA FILTRO ETICHETTE CORRETTA ---
    $tag_subquery = '';
    $tag_param = null;
    if ($tag_id_filter) {
        if ($tag_id_filter === '_any_') {
            $tag_subquery = " AND EXISTS (SELECT 1 FROM transaction_tags tt WHERE tt.transaction_id = t.id)";
        } elseif ($tag_id_filter === '_none_') {
            $tag_subquery = " AND NOT EXISTS (SELECT 1 FROM transaction_tags tt WHERE tt.transaction_id = t.id)";
        } else {
            $tag_subquery = " AND t.id IN (SELECT transaction_id FROM transaction_tags WHERE tag_id = ?)";
            $tag_param = (int)$tag_id_filter;
        }
    }
    ...
    $sql_past_tx = "SELECT SUM(amount) as total_past_tx FROM transactions t WHERE user_id = ? AND transaction_date < ?";
    $params_past = ['is', $user_id, $start_date->format('Y-m-01')];
    if (!empty($account_ids_filter)) {
        $placeholders = implode(',', array_fill(0, count($account_ids_filter), '?'));
        $sql_past_tx .= " AND t.account_id IN ($placeholders)";
        $params_past[0] .= str_repeat('i', count($account_ids_filter));
        foreach ($account_ids_filter as $acc_id) $params_past[] = $acc_id;
    }
    if ($tag_subquery) {
        $sql_past_tx .= $tag_subquery;
        if ($tag_param) {
            $params_past[0] .= 'i';
            $params_past[] = $tag_param;
        }
    }
    ...
    // Itera per ogni mese nel periodo
    foreach ($period as $dt) {
        $month_label = $dt->format('M Y');
        $month_start = $dt->format('Y-m-01');
        $month_end = $dt->format('Y-m-t');
        ...
        $sql_month_tx = "SELECT SUM(amount) as total_month_tx FROM transactions t WHERE user_id = ? AND transaction_date BETWEEN ? AND ?";
        $params_month = ['iss', $user_id, $month_start, $month_end];
        if (!empty($account_ids_filter)) {
            $placeholders = implode(',', array_fill(0, count($account_ids_filter), '?'));
            $sql_month_tx .= " AND t.account_id IN ($placeholders)";
            $params_month[0] .= str_repeat('i', count($account_ids_filter));
            foreach ($account_ids_filter as $acc_id) $params_month[] = $acc_id;
        }
        if ($tag_subquery) {
            $sql_month_tx .= $tag_subquery;
            if ($tag_param) {
                $params_month[0] .= 'i';
                $params_month[] = $tag_param;
            }
        }
...
function get_friend_debt_summary($conn, $user_id) {
    ...
    while ($row = $result->fetch_assoc()) {
        $debts[] = [
            ...
            'recurring_transaction_id' => null // Friend loans cannot be automated from here
        ];
    }
    $stmt->close();
    return $debts;
...
function send_admin_transactional_email($conn, $user_id, $email_type, $data = []) {
    // --- Recupera i dati dell'utente ---
    $sql_user = "SELECT username, email, is_verified, verification_token, receives_emails FROM users WHERE id = ? LIMIT 1";
    if (!$stmt_user = $conn->prepare($sql_user)) {
        return ['success' => false, 'message' => 'Errore preparazione query utente.'];
    }
    ...
    if ($result_user->num_rows !== 1) {
        $stmt_user->close();
        return ['success' => false, 'message' => "Utente con ID {$user_id} non trovato."];
    }
    $user = $result_user->fetch_assoc();
    $stmt_user->close();

    // Controlla se l'utente ha disattivato le email
    if (!$user['receives_emails']) {
        return ['success' => true, 'message' => "L'utente {$user['username']} ha disattivato la ricezione di email. Nessuna email inviata."];
    }
    ...
    switch ($email_type) {
        ...
        case 'verify_account':
            if ($user['is_verified']) {
                return ['success' => false, 'message' => 'Questo account è già verificato.'];
            }
        ...
        case 'custom_message':
            ...
            $params['subject_variable'] = $data['subject'] ?? 'Messaggio da Bearget';
            $params['body_variable'] = nl2br(htmlspecialchars($data['body'] ?? 'Nessun messaggio specificato.'));
            break;

        // --- NUOVO BLOCCO AGGIUNTO ---
        case 'plan_changed':
            $template_id = 10; // ID del nuovo template che devi creare in Brevo
            $params['old_plan'] = htmlspecialchars($data['old_plan'] ?? 'N/A');
            $params['new_plan'] = htmlspecialchars($data['new_plan'] ?? 'N/A');
            break;
        // --- FINE BLOCCO AGGIUNTO ---

        default:
            return ['success' => false, 'message' => 'Tipo di email non valido.'];
    }
    ...
    try {
        ...
        return ['success' => true, 'message' => "Email inviata con successo a {$user['email']}!"];
    } catch (Exception $e) {
        error_log("Errore Brevo per utente {$user_id}: " . $e->getMessage());
        return ['success' => false, 'message' => "Impossibile inviare l'email a {$user['email']}."];
    }
}

/**
 * Recupera lo stato della modalità manutenzione.
 * @param object $conn La connessione al database.
 * @return string 'on' o 'off'.
 */
function get_maintenance_status($conn) {
    $sql = "SELECT setting_value FROM app_settings WHERE setting_key = 'maintenance_mode' LIMIT 1";
    $result = $conn->query($sql);
    if ($row = $result->fetch_assoc()) {
        return $row['setting_value'];
    }
    // Fallback in caso di errore o se l'impostazione non esiste
    return 'off';
}

/**
 * Controlla se esiste una relazione di blocco tra due utenti (in entrambe le direzioni).
 * @return bool True se uno dei due ha bloccato l'altro, altrimenti false.
 */
function check_block_status($conn, $user1_id, $user2_id) {
    $sql = "SELECT id FROM user_blocks WHERE (blocker_id = ? AND blocked_id = ?) OR (blocker_id = ? AND blocked_id = ?)";
    $stmt = $conn->prepare($sql);
...
/**
 * Ottiene un array di ID degli utenti bloccati da un utente specifico.
 * @return array Un array di ID.
 */
function get_blocked_user_ids($conn, $user_id) {
    $blocked_ids = [];
