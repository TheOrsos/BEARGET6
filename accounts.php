...
<body class="text-gray-200">
    ...
    <div class="flex h-screen">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                ...
                <button onclick="openModal('add-account-modal')" class="bg-primary-600 hover:bg-primary-700 font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    Aggiungi Conto
                </button>
            </header>

            <div id="accounts-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto" style="max-height: calc(100vh - 10rem);">
                <?php if (empty($accounts)): ?>
                    <div id="empty-state" class="md:col-span-2 lg:col-span-3 bg-gray-800 rounded-2xl p-10 text-center flex flex-col items-center">
                        ...
                        <h3 class="text-lg font-semibold ">Nessun conto trovato</h3>
                        ...
                        <button onclick="openModal('add-account-modal')" class="mt-6 bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                ...
                <?php else: ?>
                    <?php foreach ($accounts as $account): ?>
                        <div class="bg-gray-800 p-6 rounded-2xl flex flex-col justify-between transition-transform duration-200 hover:-translate-y-1" data-account-id="<?php echo $account['id']; ?>">
                            <div>
                                <h3 class="text-xl font-bold  account-name"><?php echo htmlspecialchars($account['name']); ?></h3>
    ...
    <div id="add-account-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <form id="add-account-form" action="add_account.php" method="POST">
                <h2 class="text-2xl font-bold  mb-6">Nuovo Conto</h2>
                <div class="space-y-4">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-300 mb-1">Nome Conto</label>
                        <input type="text" name="name" id="name" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2" placeholder="Es. Conto Corrente">
                    </div>
                    <div>
                        <label for="initial_balance" class="block text-sm font-medium text-gray-300 mb-1">Saldo Iniziale</label>
                        <input type="number" step="0.01" name="initial_balance" id="initial_balance" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2" placeholder="0.00">
                    </div>
                </div>
                <div class="mt-8 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('add-account-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg">Salva Conto</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <form id="edit-account-form" action="update_account.php" method="POST">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold ">Modifica Conto</h2>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="edit-name" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2">
                    </div>
                    <div>
                        <label for="edit-initial_balance" class="block text-sm font-medium text-gray-300 mb-1">Saldo Iniziale</label>
                        <input type="number" step="0.01" name="initial_balance" id="edit-initial_balance" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2">
                    </div>
                </div>
                <div class="mt-8 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('edit-account-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
    ...
    <div id="confirm-delete-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content text-center">
            ...
            <h3 class="text-lg leading-6 font-bold  mt-4">Eliminare Conto?</h3>
    ...
    <script>
        ...
        function addAccountToGrid(account) {
            ...
            newCard.innerHTML = `
                <div>
                    <h3 class="text-xl font-bold  account-name">${escapeHTML(account.name)}</h3>
