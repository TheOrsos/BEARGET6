...
<body class="text-gray-200">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (!toggleFilterBtn || !filterContainer) return;

        const urlParams = new URLSearchParams(window.location.search);
        const filtersAreActive = urlParams.get('start_date') || urlParams.get('end_date') || urlParams.get('description') || urlParams.get('category_id') || urlParams.get('account_id') || urlParams.get('tag_id');

        function setButtonToHide() {
            toggleFilterBtn.innerHTML = `<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg> <span>Nascondi</span>`;
