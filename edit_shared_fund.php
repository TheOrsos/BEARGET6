...
<body class="text-gray-200 p-10">
    <div class="max-w-lg mx-auto bg-gray-800 rounded-2xl p-8">
        <h1 class="text-2xl font-bold  mb-6">Modifica Fondo: <?php echo htmlspecialchars($fund['name']); ?></h1>
        <form action="edit_shared_fund.php?id=<?php echo $fund_id; ?>" method="POST" class="space-y-4">
            <div>
                ...
                <input type="text" name="name" value="<?php echo htmlspecialchars($fund['name']); ?>" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Obiettivo di Raccolta (€)</label>
                <input type="number" step="0.01" name="target_amount" value="<?php echo $fund['target_amount']; ?>" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
            </div>
            <div class="pt-4 flex justify-end space-x-4">
                <a href="fund_details.php?id=<?php echo $fund_id; ?>" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</a>
