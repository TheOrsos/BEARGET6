...
<body class="text-gray-300">
    ...
    if (card) {
        ...
        const iconsContainer = card.querySelector('.flex.items-center.space-x-3');
        if (iconsContainer) {
            let iconsHTML = '';
            try {
                const todolist = JSON.parse(note.todolist_content || '[]');
                if (Array.isArray(todolist) && todolist.length > 0) {
                    iconsHTML += `<svg class="w-4 h-4 text-gray-400" title="Questa nota contiene una to-do list" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>`;
                }
            } catch(e) {}

            if ( (note.creator_id != CURRENT_USER_ID) || (note.creator_id == CURRENT_USER_ID && note.share_count > 0) ) {
                iconsHTML += `<svg class="w-4 h-4 text-gray-400" title="Nota condivisa" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21a6 6 0 00-9-5.197m0 0A4.992 4.992 0 004.5 13.545M12 12a4 4 0 110-8 4 4 0 010 8z"></path></svg>`;
            }
            
            if (note.linked_goal_id) {
                 iconsHTML += `<a href="goals.php#goal-${note.linked_goal_id}" title="Collegato a un obiettivo"><svg class="w-4 h-4 text-green-400 hover:text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path></svg></a>`;
            } else if (note.transaction_id) {
                 iconsHTML += `<a href="transactions.php?description=${encodeURIComponent(note.transaction_description)}" title="Collegato a una transazione"><svg class="w-4 h-4 text-blue-400 hover:text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg></a>`;
            }
