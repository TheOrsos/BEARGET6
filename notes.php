...
// Prepara i filtri dalla query string dell'URL per il caricamento iniziale
$initial_filters = [
...
$notes = get_notes_for_user($conn, $user_id, $initial_filters);
$current_page = 'notes';
?>
<!DOCTYPE html>
<html lang="it">
...
<body class="text-gray-300">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                ...
                <div class="flex items-center gap-2">
                    <button id="toggle-filter-btn" class="bg-gray-600 hover:bg-gray-500  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
                    ...
                    <button id="add-note-btn" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors shadow-lg hover:shadow-primary-500/50">
            ...
            <div id="filter-container" class="bg-gray-800 rounded-2xl p-4 mb-6 hidden transition-all duration-300">
                <form id="notes-filter-form" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                    <div class="lg:col-span-2">
                        <label for="text_search" class="text-sm font-medium text-gray-400">Titolo/Contenuto</label>
                        <input type="text" name="text_search" id="text_search" value="<?php echo htmlspecialchars($initial_filters['text_search'] ?? ''); ?>" placeholder="Cerca testo..." class="w-full bg-gray-700  rounded-lg px-3 py-2 mt-1 text-sm">
                    </div>
                    <div>
                        <label for="id_search" class="text-sm font-medium text-gray-400">ID Nota</label>
                        <input type="number" name="id_search" id="id_search" value="<?php echo htmlspecialchars($initial_filters['id_search'] ?? ''); ?>" placeholder="Es. 123" class="w-full bg-gray-700  rounded-lg px-3 py-2 mt-1 text-sm">
                    </div>
                    <div>
                        <label for="date_search" class="text-sm font-medium text-gray-400">Data Creazione</label>
                        <input type="date" name="date_search" id="date_search" value="<?php echo htmlspecialchars($initial_filters['date_search'] ?? ''); ?>" class="w-full bg-gray-700  rounded-lg px-3 py-2 mt-1 text-sm">
                    </div>
                    <div class="flex gap-2">
                        <a href="notes.php" class="w-full text-center bg-gray-600 hover:bg-gray-500  font-semibold py-2 px-4 rounded-lg">Resetta</a>
                    </div>
                </form>
            </div>

            <div id="notes-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto" style="height: calc(100vh - 12rem);">
                <?php if (empty($notes)): ?>
                ...
                <div class="relative block bg-gray-800 hover:bg-gray-700 p-6 rounded-2xl transition-colors cursor-pointer" onclick='openNoteModal(<?php echo json_encode($note); ?>)' data-note-id="<?php echo $note['id']; ?>">
                    <div class="flex justify-between items-start">
                        <h3 class="text-xl font-bold  truncate mb-2 note-title"><?php echo htmlspecialchars($note['title']); ?></h3>
                        <div class="tag-container flex items-center gap-2">
                            <?php if ($note['creator_id'] != $user_id): ?>
                                <span class="text-xs bg-green-600  font-semibold px-2 py-1 rounded-full shared-tag">Condivisa</span>
                            <?php endif; ?>
                            <?php
                                $todolist_items = json_decode($note['todolist_content'], true);
                                if (!empty($todolist_items) && is_array($todolist_items)):
                            ?>
                                <span class="text-xs bg-primary-600  font-semibold px-2 py-1 rounded-full todo-tag">To-Do</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center mt-4">
                        ...
                        <?php if (!empty($note['transaction_id'])): ?>
                            <a href="transactions.php?description=<?php echo urlencode($note['transaction_description']); ?>" class="text-xs bg-blue-600  font-semibold px-2 py-1 rounded-full hover:bg-blue-700 transition-colors" title="Collegato alla transazione: <?php echo htmlspecialchars($note['transaction_description']); ?>">
                                Collegato
                            </a>
                        <?php endif; ?>
                    </div>
            ...
            <form id="note-form" class="flex flex-col flex-grow min-h-0">
                ...
                <header class="flex justify-between items-center mb-6 flex-shrink-0 gap-4">
                    <div class="flex-grow">
                        ...
                        <input type="text" name="title" id="note-title" class="text-3xl font-bold  bg-transparent border-0 border-b-2 border-gray-700 focus:ring-0 focus:border-primary-500 w-full" placeholder="Titolo...">
                    </div>
                    <div class="flex items-center space-x-2 flex-shrink-0">
                        <button type="button" id="members-note-btn" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-4 rounded-lg hidden">Membri</button>
                        <button type="button" id="share-note-btn" class="bg-blue-600 hover:bg-blue-700  font-semibold py-2 px-4 rounded-lg hidden">Condividi</button>
                        <button type="button" id="delete-note-btn" class="bg-danger hover:bg-red-700  font-semibold py-2 px-4 rounded-lg">Elimina</button>
                        <button type="submit" id="save-note-btn" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg">Salva</button>
                    </div>
                </header>

                <div class="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
                    <div class="flex flex-col">
                        <h3 class="text-lg font-semibold  mb-2">Testo</h3>
    ...
    <div id="share-note-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h3 class="text-2xl font-bold  mb-4">Condividi Nota</h3>
            ...
            <form id="share-note-form">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="friend_code" id="friend_code" class="flex-grow bg-gray-700  rounded-lg px-3 py-2" required>
                        <button type="button" id="choose-friend-btn-note" class="bg-gray-600 hover:bg-gray-500  font-semibold py-2 px-4 rounded-lg">Scegli</button>
                    </div>
                </div>
                <div class="mb-6">
                    <label for="permission" class="block text-sm font-medium text-gray-300 mb-1">Permesso</label>
                    <select name="permission" id="permission" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        <option value="edit">Può modificare</option>
                        <option value="view">Può solo visualizzare</option>
                    </select>
                </div>
                <div class="flex justify-end gap-4">
                    <button type="button" class="text-gray-400 hover:" onclick="closeModal('share-note-modal')">Annulla</button>
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg">Condividi</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="friends-list-modal-note" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h3 class="text-2xl font-bold  mb-4">Scegli un Amico</h3>
            <div class="mb-4">
                <input type="text" id="friend-search-input-note" placeholder="Cerca per nome utente..." class="w-full bg-gray-700  rounded-lg px-3 py-2 text-sm border border-gray-600 focus:ring-primary-500 focus:border-primary-500">
            </div>
            ...
            <div class="flex justify-end mt-6">
                <button type="button" class="text-gray-400 hover:" onclick="closeModal('friends-list-modal-note')">Chiudi</button>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderNotes(notes) {
            ...
            notes.forEach(note => {
                ...
                noteCard.className = 'relative block bg-gray-800 hover:bg-gray-700 p-6 rounded-2xl transition-colors cursor-pointer';
                noteCard.dataset.noteId = note.id;
                noteCard.onclick = () => openNoteModal(note);

                let tagsHTML = '';
                if (note.creator_id != CURRENT_USER_ID) {
                    tagsHTML += `<span class="text-xs bg-green-600 font-semibold px-2 py-1 rounded-full shared-tag">Condivisa</span>`;
                }
                const todoItems = JSON.parse(note.todolist_content || '[]');
                if (Array.isArray(todoItems) && todoItems.length > 0) {
                    tagsHTML += `<span class="text-xs bg-primary-600 font-semibold px-2 py-1 rounded-full todo-tag">To-Do</span>`;
                }

                let transactionLink = '';
                if(note.transaction_id) {
                    transactionLink = `<a href="transactions.php?description=${encodeURIComponent(note.transaction_description)}" class="text-xs bg-blue-600 font-semibold px-2 py-1 rounded-full hover:bg-blue-700 transition-colors" title="Collegato alla transazione: ${escapeHTML(note.transaction_description)}">Collegato</a>`;
                }

                noteCard.innerHTML = `
                    <div class="flex justify-between items-start">
                        <h3 class="text-xl font-bold truncate mb-2 note-title">${escapeHTML(note.title)}</h3>
                        <div class="tag-container flex items-center gap-2">${tagsHTML}</div>
                    </div>
                    <p class="text-gray-400 text-sm h-12 overflow-hidden note-content-preview">
                    ...
                    <div class="flex justify-between items-center mt-4">
                        <p class="text-xs text-gray-500 note-date">
                            Modificato: ${new Date(note.updated_at).toLocaleDateString('it-IT')}
                        </p>
                        ${transactionLink}
                    </div>`;
        ...
        async function updateNotesList() {
            ...
            const url = `api_search_notes.php?${params.toString()}`;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    notesGrid.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-red-400">Errore: ${data.message}</td></tr>`;
                }
            } catch (error) {
                notesGrid.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-red-400">Errore di connessione.</td></tr>`;
            }
        }
        ...
        inputs.forEach(input => {
            input.addEventListener('input', () => {
                ...
                debounceTimer = setTimeout(updateNotesList, 350);
            });
        });
    });
    </script>
    ...
    <script>
        // --- SCRIPT ESISTENTE PER LA LOGICA DEI MODALI ---
...
function setupFriendSelector(prefix) {
        ...
        function createTodoItem(task = '', completed = false) {
            ...
            const textClasses = `flex-grow bg-transparent  border-0 focus:ring-0 mx-2 ${completed ? 'line-through text-gray-500' : ''}`;
