...
<body class="text-gray-300">
    ...
    <script>
    // --- NUOVA LOGICA PER GESTIONE CONTRIBUTI ---
    document.addEventListener('DOMContentLoaded', function() {
                ...
                fetch(editContributionForm.action, { method: 'POST', body: formData })
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Ricarica la pagina per aggiornare il totale del fondo e altri dati
        ...
        if(confirmDeleteBtn) {
            confirmDeleteBtn.addEventListener('click', function() {
                if (formToDelete) {
                    ...
                    const row = formToDelete.closest('.contribution-row');
                    fetch(formToDelete.action, { method: 'POST', body: formData })
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                setTimeout(() => {
                                    row.remove();
                                    if (contributionsList.getElementsByClassName('contribution-row').length === 0) {
                                        const emptyState = document.getElementById('empty-state-contributions');
                                        if(emptyState) emptyState.style.display = 'block';
                                    }
                                    // Ricarica la pagina per aggiornare i totali
                                    window.location.reload();
                                }, 500);
                            } else {
                                alert('Errore: ' + data.message);
        ...
        // --- LOGICA NOTE (da transactions.php) ---
