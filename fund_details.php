...
<body class="text-gray-300">
    <div class="flex h-screen">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                <div>
                    <h1 class="text-3xl font-bold "><?php echo htmlspecialchars($fund['name']); ?></h1>
                ...
                <div class="flex gap-2" style="flex-wrap: wrap;">
                    <a href="shared_funds.php" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-4 rounded-lg flex items-center">Indietro</a>
                    <a href="fund_stats.php?id=<?php echo $fund_id; ?>" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-4 rounded-lg flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                        Statistiche
                    </a>
                    <?php if (!$is_creator): ?>
                        <button id="leave-fund-btn" class="bg-red-800 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center transition-colors">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                            Abbandona Fondo
                        </button>
                    <?php endif; ?>
                    <?php if($fund['status'] === 'active' && $is_creator): ?>
                        <button onclick="openModal('settle-up-modal')" class="bg-red-600 hover:bg-red-700  font-semibold py-2 px-4 rounded-lg flex items-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            Chiudi Conto
                        </button>
                    <?php elseif($fund['status'] === 'settling' && $is_creator && $all_payments_confirmed): ?>
                        <button onclick="openModal('archive-fund-modal')" class="bg-green-600 hover:bg-green-700  font-semibold py-2 px-4 rounded-lg flex items-center">
                    ...
                    <?php if($fund['status'] === 'active'): ?>
                        <button onclick="openModal('add-expense-modal')" class="bg-blue-600 hover:bg-blue-700  font-semibold py-2 px-4 rounded-lg flex items-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            Spesa
                        </button>
                        <button onclick="openModal('add-contribution-modal')" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-4 rounded-lg flex items-center">
            ...
            <?php if ($fund['status'] === 'settling'): ?>
                <div class="bg-gray-800 rounded-2xl p-6">
                    <h2 class="text-xl font-bold  mb-4">Azioni per il Saldaconto</h2>
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-3 rounded-lg bg-gray-700/50">
                            ...
                            <form action="confirm_payment.php" method="POST" class="confirm-payment-form">
                                ...
                                <button type="submit" class="bg-green-600 hover:bg-green-700  font-semibold py-1 px-3 rounded-lg text-sm">
            ...
            ?>
                <div class="bg-gray-800 rounded-2xl p-6">
                    <h2 class="text-xl font-bold  mb-2">Saldaconto Automatico</h2>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <?php if ($user_id == $payment['from_user_id'] && empty($payment['from_account_id'])): ?>
                                                <select data-payment-id="<?php echo $payment['id']; ?>" data-type="from" class="account-select w-full bg-gray-900  rounded-lg px-3 py-2">
                                        ...
                                        <div>
                                            ...
                                            <?php if ($user_id == $payment['to_user_id'] && empty($payment['to_account_id'])): ?>
                                                <select data-payment-id="<?php echo $payment['id']; ?>" data-type="to" class="account-select w-full bg-gray-900  rounded-lg px-3 py-2">
                                                    <option value="">Scegli un conto...</option>
                    ...
                    <div class="mt-6 text-right">
                        <form action="process_automatic_settlement.php" method="POST">
                            ...
                            <button type="submit" class="bg-green-600 hover:bg-green-700  font-semibold py-2 px-5 rounded-lg">Processa e Archivia Fondo</button>
                        </form>
                    </div>
                    <?php endif; ?>
                </div>
            <?php else: // active or archived ?>
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-gray-800 rounded-2xl p-6">
                            <h2 class="text-xl font-bold  mb-4">Riepilogo</h2>
                            <?php
                                $percentage = ($fund['target_amount'] > 0) ? ($fund['total_contributed'] / $fund['target_amount']) * 100 : 0;
                            ?>
                            <div class="w-full bg-gray-700 rounded-full h-4 mb-2">
                                <div class="bg-green-500 h-4 rounded-full text-center  text-xs font-bold" style="width: <?php echo min($percentage, 100); ?>%"><?php echo round($percentage); ?>%</div>
                            </div>
                            <div class="flex justify-between text-lg text-gray-300">
                                <span class="font-bold ">€<?php echo number_format($fund['total_contributed'], 2, ',', '.'); ?></span>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center justify-between p-2 rounded-lg transition-colors hover:bg-gray-700/50">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-semibold "><?php echo htmlspecialchars($expense['description']); ?></p>
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <a href="note_details.php?id=<?php echo $expense['note_id']; ?>" class="text-gray-400 hover:" title="Visualizza nota">
                            ...
                            <div id="contributions-list" class="space-y-2">
                                ...
                                <div class="contribution-row flex items-center justify-between p-2 rounded-lg transition-colors hover:bg-gray-700/50" data-contribution-id="<?php echo $c['id']; ?>">
                                    <div class="contribution-details">
                                        <p class="font-semibold  username-cell"><?php echo htmlspecialchars($c['username']); ?></p>
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        <div class="bg-gray-800 rounded-2xl p-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center p-2 rounded-lg transition-colors hover:bg-gray-700/50">
                                    <div class="w-8 h-8 rounded-full bg-primary-600 flex items-center justify-center  font-bold text-sm mr-3 flex-shrink-0"><?php echo strtoupper(substr($member['username'], 0, 1)); ?></div>
                        ...
                        <!-- Balances -->
                        <div class="bg-gray-800 rounded-2xl p-6">
                            <h2 class="text-xl font-bold  mb-4">Bilanci</h2>
                            <?php
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="flex items-center justify-between p-1">
                                        <span class=""><?php echo htmlspecialchars($balance['username']); ?></span>
                        ...
                        <div class="bg-gray-800 rounded-2xl p-6">
                            <h2 class="text-xl font-bold  mb-4">Invita Membro</h2>
                            <form action="invite_member.php" method="POST" class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <input type="text" name="friend_code" id="friend_code_fund" required class="flex-grow bg-gray-700  rounded-lg px-3 py-2" placeholder="ABC123DE">
                                        <button type="button" id="choose-friend-btn-fund" class="bg-gray-600 hover:bg-gray-500  font-semibold py-2 px-4 rounded-lg">Scegli</button>
                                    </div>
                                </div>
                                <button type="submit" class="w-full bg-primary-600 hover:bg-primary-700  font-semibold py-2 rounded-lg flex items-center justify-center">
    ...
    <div id="add-expense-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-lg p-6 transform scale-95 opacity-0 modal-content overflow-y-auto max-h-full">
            <h2 class="text-2xl font-bold  mb-6">Aggiungi Nuova Spesa</h2>
            <form id="add-expense-form" action="add_expense.php" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="description" required class="w-full bg-gray-700  rounded-lg px-3 py-2" placeholder="Es. Cena fuori">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Importo Totale (€)</label>
                        <input type="number" step="0.01" name="amount" id="expense-amount" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Data Spesa</label>
                        <input type="date" name="expense_date" required class="w-full bg-gray-700  rounded-lg px-3 py-2" value="<?php echo date('Y-m-d'); ?>">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1">Pagato da</label>
                    <select name="paid_by_user_id" id="paid-by-user-id-select" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        <?php foreach($members as $member): ?>
...
<div id="account-id-container">
    ...
    <select name="account_id" id="account-id-select" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
        ...
        <?php endforeach; ?>
                ...
                <div>
                    <h3 class="text-lg font-medium  mb-2 mt-4">Divisione Spesa</h3>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Metodo di divisione</label>
                        <select name="split_method" id="split-method-select" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                ...
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('add-expense-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" id="add-expense-submit-btn" class="bg-blue-600 hover:bg-blue-700  font-semibold py-2 px-5 rounded-lg">Aggiungi Spesa</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="add-contribution-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h2 class="text-2xl font-bold  mb-6">Versa nel Fondo</h2>
            <form action="add_fund_contribution.php" method="POST" class="space-y-4">
                ...
                <?php endif; ?>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('add-contribution-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" class="bg-green-600 hover:bg-green-700  font-semibold py-2 px-5 rounded-lg">Conferma</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <?php if($fund['status'] === 'active' && $is_creator): ?>
        ...
        <div id="settle-up-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
             ...
             <div class="bg-gray-800 rounded-lg p-6 z-10 max-w-md text-center shadow-lg">
                <h2 class="text-xl font-bold mb-4 ">Chiudere il Conto?</h2>
                ...
                <form action="calculate_settlement.php" method="POST">
                    ...
                    <div class="mt-6 flex justify-center gap-4">
                        <button type="button" onclick="closeModal('settle-up-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                        <input type="hidden" name="fund_id" value="<?php echo $fund_id; ?>">
                        <button type="submit" class="bg-red-600 hover:bg-red-700  font-semibold py-2 px-5 rounded-lg">Sì, chiudi conto</button>
                    </div>
                </form>
             </div>
        </div>
    ...
    <?php if($fund['status'] === 'settling' && $is_creator && $all_payments_confirmed): ?>
        ...
        <div id="archive-fund-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
            <div class="fixed inset-0 bg-black bg-opacity-60" onclick="closeModal('archive-fund-modal')"></div>
             <div class="bg-gray-800 rounded-lg p-6 z-10 max-w-md text-center shadow-lg">
                <h2 class="text-xl font-bold mb-4 ">Archiviare il Fondo?</h2>
                ...
                <div class="mt-6 flex justify-center gap-4">
                    <button type="button" onclick="closeModal('archive-fund-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <form action="archive_fund.php" method="POST">
                        <input type="hidden" name="fund_id" value="<?php echo $fund_id; ?>">
                        <button type="submit" class="bg-green-600 hover:bg-green-700  font-semibold py-2 px-5 rounded-lg">Sì, archivia</button>
                    </form>
                </div>
             </div>
        </div>
    ...
    <div id="edit-contribution-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg p-6 transform scale-95 opacity-0 modal-content">
            <h2 class="text-2xl font-bold  mb-6">Modifica Contributo</h2>
            <form id="edit-contribution-form" action="update_fund_contribution.php" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Importo (€)</label>
                        <input type="number" step="0.01" name="amount" id="edit-contribution-amount" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Data</label>
                        <input type="date" name="contribution_date" id="edit-contribution-date" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-300 mb-1">Dal tuo conto</label>
                        <select name="account_id" id="edit-contribution-account" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                            <?php foreach($accounts as $account): ?>
                                <option value="<?php echo $account['id']; ?>"><?php echo htmlspecialchars($account['name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="mt-8 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('edit-contribution-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
    ...
    <div id="confirm-delete-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content text-center">
            ...
            <h3 class="text-lg leading-6 font-bold  mt-4">Eliminare Contributo?</h3>
            ...
            <div class="mt-8 flex justify-center space-x-4">
                <button id="confirm-delete-btn" type="button" class="bg-red-500 hover:bg-red-700  font-semibold py-2 px-5 rounded-lg">Elimina</button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
                    ...
                    case 'equal':
                        ...
                        members.forEach(member => {
                            html += `<label class="flex items-center bg-gray-700 p-2 rounded-lg">
                                ...
                                <span class="ml-2 ">${escapeHTML(member.username)}</span>
                            </label>`;
                        });
                        html += `</div>`;
                        break;
                    ...
                    case 'percentage':
                        ...
                        members.forEach(member => {
                            html += `<div class="flex items-center justify-between">
                                <label class="">${escapeHTML(member.username)}</label>
                                <div class="flex items-center w-1/2">
                                    <input type="number" step="0.01" name="${method}[${member.id}]" class="split-input w-full bg-gray-900  rounded-lg px-3 py-1" placeholder="0.00" data-method="${method}">
        ...
        // --- Inizio Logica per Abbandonare Fondo ---
        const leaveFundBtn = document.getElementById('leave-fund-btn');
        if (leaveFundBtn) {
            leaveFundBtn.addEventListener('click', function() {
                const fundId = <?php echo json_encode($fund_id); ?>;
                const fundStatus = <?php echo json_encode($fund['status']); ?>;
                
                let message = "Sei sicuro di voler abbandonare questo fondo? Questa azione non può essere annullata.";
                if (fundStatus === 'active') {
                    message = "Sei sicuro? Potrai abbandonare il fondo solo se il tuo saldo è pari a zero. L'azione non può essere annullata.";
                }

                // Usa la funzione globale che abbiamo definito nel footer
                showConfirmationModal(message, function() {
                    fetch('leave_shared_fund.php', {
                        method: 'POST',
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Dopo aver mostrato il messaggio, reindirizza l'utente alla lista dei fondi
                            setTimeout(() => {
                                window.location.href = 'shared_funds.php';
                            }, 1500);
                        } else {
                            showToast(data.message, 'error');
                        }
                    })
                    .catch(err => showToast('Si è verificato un errore di rete.', 'error'));
                }, "Conferma Abbandono Fondo");
            });
        }
        // --- Fine Logica per Abbandonare Fondo ---
        ...
        if (params.has('message')) {
            ...
            // La funzione showToast() dovrebbe essere disponibile dal footer
            // Usiamo un piccolo timeout per essere sicuri che la pagina sia pronta
            setTimeout(() => {
                showToast(message, type);
            }, 100);

            // Pulisce l'URL per non mostrare di nuovo il messaggio al refresh,
            // mantenendo l'ID del fondo.
            if (window.history.replaceState) {
                const cleanUrl = window.location.pathname + '?id=' + <?php echo json_encode($fund_id); ?>;
