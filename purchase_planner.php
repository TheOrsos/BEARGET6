...
<body class="text-gray-200">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-gray-800 rounded-2xl p-6">
                    ...
                    <div class="mt-8 border-t border-gray-700 pt-6">
                         <button id="analyze-button" class="w-full bg-green-600 hover:bg-green-700  font-bold py-3 px-5 rounded-lg flex items-center justify-center transition-colors text-lg disabled:bg-gray-600 disabled:cursor-not-allowed" disabled>
    ...
    <div id="purchase-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-lg p-6 transform scale-95 opacity-0 modal-content overflow-y-auto max-h-screen">
            <h2 class="text-2xl font-bold  mb-6">Aggiungi un Desiderio</h2>
            <form id="purchase-form" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1">Nome Oggetto</label>
                    <input type="text" id="purchase-name" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Costo Totale (€)</label>
                        <input type="number" step="0.01" id="purchase-total-cost" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">Data Obiettivo</label>
                        <input type="date" id="purchase-date" value="<?php echo date('Y-m-d'); ?>" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                    </div>
                </div>
                <div class="border-t border-b border-gray-700 py-4">
                    <div class="flex items-center justify-between">
                        <label for="is-goal" class="font-medium  flex-grow">Crea un Obiettivo di Risparmio?</label>
                ...
                <div id="installment-section">
                    <h3 class="text-lg font-semibold  mb-2">Dettagli Acquisto Diretto (Opzionale)</h3>
                    <div class="grid grid-cols-2 gap-4">
                         <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Anticipo</label>
                            <input type="number" step="0.01" id="purchase-down-payment" placeholder="0.00" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Costi Una Tantum</label>
                            <input type="number" step="0.01" id="purchase-additional-costs" placeholder="0.00" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">N. di Rate</label>
                            <input type="number" id="purchase-months" placeholder="0" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        </div>
                         <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Importo Rata (€)</label>
                            <input type="number" step="0.01" id="purchase-monthly-cost" placeholder="0.00" class="w-full bg-gray-700  rounded-lg px-3 py-2">
                        </div>
                    </div>
                </div>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('purchase-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg">Aggiungi alla Lista</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="info-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            ...
            <div class="mt-6 text-right">
                <button type="button" onclick="closeModal('info-modal')" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg">Chiudi</button>
            </div>
        </div>
    </div>
            ...
            function renderWishlist() {
                ...
                if (wishlist.length === 0) {
                    ...
                    wishlist.forEach((item, index) => {
                        ...
                        itemEl.innerHTML = `<div><span class="font-semibold ">${escapeHTML(item.name)}</span><span class="text-xs text-gray-400"> - ${formattedDate}</span><span class="text-xs font-medium px-2 py-1 rounded-full ml-2 ${typeClass}">${typeLabel}</span></div><div class="flex items-center gap-4"><span class="font-bold ">€${item.cost.toFixed(2)}</span><button type="button" class="text-gray-500 hover:text-red-400" data-index="${index}">&times;</button></div>`;
            ...
            analyzeButton.addEventListener('click', function() {
                ...
                .then(data => {
                    ...
                    if (data.ongoing_goals && data.ongoing_goals.length > 0) {
                        analysisHtml += `<div class="mt-6 border-t border-gray-700 pt-4"><h3 class="text-lg font-semibold  mb-3">Ricorda i tuoi impegni</h3><div class="space-y-2">`;
                        data.ongoing_goals.forEach(goal => {
                            ...
                            analysisHtml += `<div class="flex items-center justify-between text-sm text-gray-400">
                                                ...
                                                <button class="info-button inline-flex items-center px-2 py-1 bg-primary-600  text-xs font-medium rounded-md hover:bg-primary-700 transition-colors" data-title="Dettaglio: ${escapeHTML(goal.name)}" data-details="${escapeHTML(detailText)}">
