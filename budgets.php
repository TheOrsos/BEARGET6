...
<body class="text-gray-200">
    ...
    <div class="flex h-screen">
        ...
        <main class="flex-1 p-6 lg:p-10 overflow-y-auto">
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold  flex items-center gap-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"></path></svg>
                            Budget mensile
                        </h1>
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex items-center bg-gray-800 rounded-lg p-1">
                        <a href="?view=list" class="px-4 py-1.5 text-sm font-semibold rounded-md transition-colors <?php echo $view === 'list' ? 'bg-primary-600 ' : 'text-gray-400 hover:bg-gray-700'; ?>">Lista</a>
                        <a href="?view=graph" class="px-4 py-1.5 text-sm font-semibold rounded-md transition-colors <?php echo $view === 'graph' ? 'bg-primary-600 ' : 'text-gray-400 hover:bg-gray-700'; ?>">Grafico</a>
                    </div>
                    <button onclick="openModal('add-budget-modal')" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
            ...
            <div id="list-view" class="space-y-4 <?php echo $view !== 'list' ? 'hidden' : ''; ?> overflow-y-auto" style="max-height: calc(100vh - 10rem);">
                <?php if (empty($budgets)): ?>
                    <div id="empty-state-budgets" class="bg-gray-800 rounded-2xl p-10 text-center flex flex-col items-center">
                        ...
                        <h3 class="text-lg font-semibold ">Nessun budget impostato</h3>
                        ...
                        <button onclick="openModal('add-budget-modal')" class="mt-6 bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    ...
                    <div class="bg-gray-800 rounded-2xl p-5 transition-transform duration-200 hover:-translate-y-1" data-budget-id="<?php echo $budget['id']; ?>">
                        <div class="flex justify-between items-center mb-2">
                            <div class="flex items-center">
                                ...
                                <span class="font-bold  category-name"><?php echo htmlspecialchars($budget['category_name']); ?></span>
                            </div>
            ...
            <div id="graph-view" class="bg-gray-800 rounded-2xl p-6 <?php echo $view !== 'graph' ? 'hidden' : ''; ?>">
                <h2 class="text-xl font-bold  mb-4">Confronto Budget vs. Speso</h2>
    ...
    <div id="add-budget-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h2 class="text-2xl font-bold  mb-4">Imposta Nuovo Budget</h2>
            <form id="add-budget-form" action="add_budget.php" method="POST" class="space-y-4">
                <div>
                    ...
                    <select name="category_id" id="category_id" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" id="amount" required placeholder="Es. 150.00" class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2">
                </div>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('add-budget-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2.5 px-5 rounded-lg transition-colors" <?php if (empty($availableCategories)) echo 'disabled'; ?>>
                        Salva Budget
                    </button>
                </div>
            </form>
    ...
    <div id="edit-budget-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-2xl font-bold ">Modifica Budget</h2>
            ...
            <form id="edit-budget-form" action="update_budget.php" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" id="edit-amount" required class="w-full bg-gray-700 border-gray-600  rounded-lg px-3 py-2">
                </div>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('edit-budget-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
                    <button type="submit" class="bg-green-600 hover:bg-green-700  font-semibold py-2.5 px-5 rounded-lg transition-colors">
                        Salva Modifiche
                    </button>
                </div>
            </form>
    ...
    <div id="confirm-delete-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content text-center">
            ...
            <h3 class="text-lg leading-6 font-bold  mt-4">Eliminare Budget?</h3>
                ...
                newCard.innerHTML = `
                    <div class="flex justify-between items-center mb-2">
                        <div class="flex items-center">
                            ...
                            <span class="font-bold  category-name">${escapeHTML(budget.category_name)}</span>
                        </div>
                        <div class="flex items-center space-x-2">
