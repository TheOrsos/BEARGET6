...
<body class="text-gray-300">
            ...
            <header class="flex flex-wrap justify-between items-center gap-4 mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-400 mt-1">Organizza le spese con i tuoi amici.</p>
                    </div>
                </div>
                <button onclick="openModal('add-fund-modal')" class="bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors shadow-lg hover:shadow-primary-500/50">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    Crea Fondo
                </button>
            </header>

            <div id="funds-grid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 overflow-y-auto" style="max-height: calc(100vh - 10rem);">
                <?php if (empty($shared_funds)): ?>
                    <div id="empty-state-funds" class="md:col-span-2 xl:col-span-3 bg-gray-800 rounded-2xl p-10 text-center flex flex-col items-center">
                        ...
                        <h3 class="text-lg font-semibold ">Non fai parte di nessun fondo comune</h3>
                        ...
                        <button onclick="openModal('add-fund-modal')" class="mt-6 bg-primary-600 hover:bg-primary-700  font-semibold py-2 px-5 rounded-lg flex items-center transition-colors shadow-lg hover:shadow-primary-500/50">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                ...
                <div class="bg-gray-800 rounded-2xl p-5 flex flex-col transition-transform duration-200 hover:-translate-y-1 hover:shadow-2xl" data-fund-id="<?php echo $fund['id']; ?>">
                    <div class="flex-grow">
                        <div class="flex justify-between items-start">
                             <h3 class="text-xl font-bold  mb-2 fund-name"><?php echo htmlspecialchars($fund['name']); ?></h3>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-700">
                        <a href="fund_details.php?id=<?php echo $fund['id']; ?>" class="w-full text-center block bg-gray-700 hover:bg-gray-600  font-semibold py-2 rounded-lg transition-colors">Visualizza Dettagli</a>
                    </div>
                </div>
    ...
    <div id="add-fund-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <h2 class="text-2xl font-bold  mb-6">Crea Nuovo Fondo Comune</h2>
            <form id="add-fund-form" action="add_shared_fund.php" method="POST" class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" required class="w-full bg-gray-700  rounded-lg px-3 py-2" placeholder="Es. Vacanza in Sicilia">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1">Obiettivo di Raccolta (€)</label>
                    <input type="number" step="0.01" name="target_amount" required class="w-full bg-gray-700  rounded-lg px-3 py-2" placeholder="Es. 1000.00">
                </div>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('add-fund-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
    ...
    <div id="edit-fund-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold ">Modifica Fondo Comune</h2>
            ...
            <form id="edit-fund-form" action="update_shared_fund.php" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1">Nome del Fondo</label>
                    <input type="text" id="edit-fund-name" name="name" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1">Obiettivo di Raccolta (€)</label>
                    <input type="number" step="0.01" id="edit-fund-target" name="target_amount" required class="w-full bg-gray-700  rounded-lg px-3 py-2">
                </div>
                <div class="pt-4 flex justify-end space-x-4">
                    <button type="button" onclick="closeModal('edit-fund-modal')" class="bg-gray-600 hover:bg-gray-700  font-semibold py-2 px-5 rounded-lg">Annulla</button>
    ...
    <div id="confirm-delete-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 transform scale-95 opacity-0 modal-content text-center">
            ...
            <h3 class="text-lg leading-6 font-bold  mt-4">Eliminare Fondo Comune?</h3>
    ...
    <script>
        ...
        function addFundToGrid(fund) {
            ...
            newFundCard.innerHTML = `
                <div class="flex-grow">
                    <div class="flex justify-between items-start">
                         <h3 class="text-xl font-bold  mb-2 fund-name">${escapeHTML(fund.name)}</h3>
                         ${adminButtons}
                    </div>
                    <div class="w-full bg-gray-700 rounded-full h-2.5 mb-2">
                ...
                <div class="mt-4 pt-4 border-t border-gray-700">
                    <a href="fund_details.php?id=${fund.id}" class="w-full text-center block bg-gray-700 hover:bg-gray-600  font-semibold py-2 rounded-lg transition-colors">Visualizza Dettagli</a>
                </div>
            `;
